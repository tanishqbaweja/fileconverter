...
| P-04 | Broadest technically practical mainstream format coverage | Partially implemented | 389 public passed profiles across all named categories in `TESTED.md`; exact gaps are listed at its end; HEIF/HEIC, camera RAW, and SVG now have explicit evidence-backed boundaries | Implement or defensibly reject the remaining media combinations below. |
...
| P-06 | Every public profile remains at or below 250 MiB complete-Chromium incremental private memory | Verified complete for the current registry | `npm run audit:public-evidence` now proves 389/389 have strict passing reports, three-run evidence, and evidence at the published maximum; `scripts/memory-profile.mjs` uses the required process-tree formula and per-process sampling | Apply the same gate to every new route. |
...
| M-04 | Mainstream containers and practical codecs named by the specification | Partially implemented | Extensive MKV/MP4/MOV/3GP/MPEG-TS/FLV/AVI/WebM/OGV and H.264/HEVC/VP8/VP9/AV1/MPEG-2/MPEG-4 routes are public. The accepted compatible-WebM path losslessly packet-copies Matroska AV1, VP8, or VP9 plus Opus/Vorbis. The public `mkv-to-ogv` path losslessly packet-copies Theora plus optional Vorbis through isolated native profile 36; exact small-browser, write-failure, cancellation after 128 MiB, three-run 136,906,650-byte stress, 211.3 MiB memory, full-decode, cleanup, and exact no-Docker reproduction gates pass in `evidence/compatible-ogv-copy-2026-09-07.json`. `evidence/avi-output-feasibility-2026-09-08.json` records the hidden bounded-AVI profile-37 candidate: a project-owned FFmpeg 8.1.2 option rolls OpenDML RIFF segments at 8 MiB, caps the theoretical retained packet-index payload near 16 MiB, reserves a 128-GiB master-index range, keeps 256 KiB I/O/one pending write, and accepts only MPEG-4 Part 2 plus MP3. Its source-patch and planner gates pass and no converted file was created. | Finish the no-Docker AVI Wasm rebuild and production-browser success/adverse/field/three-run memory certification before exposing `mkv-to-avi`. Then investigate the remaining 3GP, AV1, MPEG-2 audio/codec combinations and elementary/raw outputs listed in `TESTED.md`. |
...
1. Expand the passing Edge, Brave, and Opera smoke evidence into representative
   route and headed interaction coverage.
2. Perform bounded feasibility work for the missing media combinations. Revisit
...
- Current result: 389 public passed profiles, 389 with three-run evidence, 389
