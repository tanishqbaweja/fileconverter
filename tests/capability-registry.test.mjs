...
test("registry has no stale pending candidates and keeps failed evidence hidden", () => {
  assert.deepEqual(
    conversionProfiles
  ...
  assert.equal(promoted.automatedTestStatus, "passed");
  assert.equal(promoted.maxTestedBytes, 170_433_726);
  assert.equal(
    publicProfilesFor("mp4").some((profile) => profile.id === promoted.id),
    true,
  );
  assert.equal(
    publicProfilesFor("mp4", true).some(
      (profile) => profile.id === promoted.id,
    ),
    true,
  );
...
test("TXT to ODT is public only at its measured evidence limit", () => {
  const profile = conversionProfiles.find(
    (candidate) => candidate.id === "txt-to-odt",
  );
  assert.ok(profile);
  assert.equal(profile.public, true);
  assert.equal(profile.automatedTestStatus, "passed");
...
test("TXT to EPUB is public only at its measured evidence limit", () => {
  const profile = conversionProfiles.find(
    (candidate) => candidate.id === "txt-to-epub",
  );
  assert.ok(profile);
  assert.equal(profile.public, true);
  assert.equal(profile.automatedTestStatus, "passed");
...
test("compatible WebM stream copy is public after its measured evidence passes", () => {
  const profile = conversionProfiles.find(
...
test("every FFmpeg profile is declared by the reproducible Wasm manifest", () => {
  ...
  assert.deepEqual(manifest.modules, [
    ...
    },
    {
      ...
      initialWasmMemoryBytes: 24 * 1024 * 1024,
      maximumWasmMemoryBytes: 64 * 1024 * 1024,
      wasmGrowthStepBytes: 4 * 1024 * 1024,
      routeSpecialized: true,
      enabledDemuxers: ["matroska"],
      enabledMuxers: ["mp4"],
      enabledParsers: ["aac", "h264", "hevc"],
      enabledBitstreamFilters: ["aac_adtstoasc"],
...
test("animated GIF outputs expose only routes with complete browser evidence", () => {
  ...
  for (const [id, bytes] of expected) {
    const profile = conversionProfiles.find((candidate) => candidate.id === id);
    assert.ok(profile, `missing ${id}`);
    assert.equal(profile.output, "gif", id);
    assert.equal(profile.public, true, id);
    assert.equal(profile.automatedTestStatus, "passed", id);
    assert.equal(profile.maxTestedBytes, bytes, id);
  }
