...
type StandaloneAiffInput =
  "m4a" | "aac" | "amr" | "mp3" | "flac" | "wav" | "wma" | "ogg" | "opus";
...
type ContainerAiffInput =
  "mkv" | "mp4" | "mov" | "mpeg-ts" | "flv" | "avi" | "ogv" | "webm";
...
type LegacyContainerAacOutputInput = Extract<
  ContainerAmrOutputInput,
  "avi" | "ogv"
>;
...
type ContainerLossyAudioInput =
  "mp4" | "mov" | "mpeg-ts" | "flv" | "avi" | "ogv";
...
const containerM4aEvidence: Record<
  keyof typeof containerM4aSourceCodec,
  number
> = {
  avi: 159_500_442,
  ogv: 137_218_662,
...
type StandaloneOpusOutputInput = Exclude<StandaloneAiffInput, "opus"> | "aiff";
...
type StandaloneVorbisOutputInput =
  Exclude<StandaloneAiffInput, "ogg"> | "aiff" | "amr-wb";
...
type StandaloneWmaInput =
  "m4a" | "aac" | "amr-wb" | "mp3" | "aiff" | "ogg" | "opus";

function standaloneWmaOutputProfile(
  input: StandaloneWmaInput,
): ConversionProfile {
...
type ContainerWmaInput =
  "mkv" | "mp4" | "mov" | "3gp" | "mpeg-ts" | "flv" | "avi" | "ogv" | "webm";
...
function containerWmaOutputProfile(
  input: ContainerWmaInput,
): ConversionProfile {
  const inputDescription = {
    mkv: "AAC in Matroska",
    mp4: "AAC in MP4",
...
export const conversionProfiles: readonly ConversionProfile[] = (
  [
    {
      ...
      memoryClass: "bounded-low",
      metadataLimitations: [
        "GZIP stores a single byte stream, not a directory tree.",
      ],
      fidelityLimitations: [],
    ...
    },
    {
      ...
      memoryClass: "bounded-medium",
      metadataLimitations: [
        "Only the first H.264 or MPEG-4 Part 2 video stream is converted; MP4 audio, additional video streams, subtitles, attached pictures, data, and chapters are explicitly excluded.",
        "Variable frame timing is normalized to the source average frame rate, or to the selected lower frame-rate cap without frame duplication or upconversion.",
        "MP4 general, stream, language, disposition, display-rotation, and container metadata that OGV cannot represent reliably are explicitly excluded.",
      ],
      fidelityLimitations: [
        "Video is decoded, downscaled without upscaling to at most 640 pixels wide, and encoded as lossy Theora using quality-based VBR and the fastest certified speed level 2.",
        "Automatic quality uses Theora quality 7; the bounded controls can select smaller-file quality 4, balanced quality 7, or higher quality 9, plus a lower width or frame-rate cap.",
    ...
    ...(["mp4", "mov", "3gp", "mpeg-ts"] as const).map(
      (input): ConversionProfile => ({
        ...
        metadataLimitations: [
          ...
          ...(input === "mp4"
            ? [
                "For a selected destination, MP4-to-AVI first writes the random-access OpenDML mux into browser-private storage, checks estimated quota, then copies it through one backpressured 256 KiB buffer and deletes the temporary file. This is faster and uses less complete-browser memory than direct random-access writes.",
              ]
            : []),
        ],
        fidelityLimitations: [
  ...
  ] satisfies readonly ConversionProfile[]
).map(withBoundedAudioArtworkDisclosure);
...
export function detectFormat(file: Pick<File, "name" | "type">): string {
  ...
  return (
    formats.find((format) => format.extensions.includes(extension as never))
      ?.id ??
    formats.find((format) => format.mimeTypes.includes(file.type as never))
      ?.id ??
    "binary"
  );
}
