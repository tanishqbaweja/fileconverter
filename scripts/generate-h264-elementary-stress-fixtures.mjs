import { createHash } from "node:crypto";
import { execFile } from "node:child_process";
import { createReadStream } from "node:fs";
import { mkdir, readFile, rm, stat, writeFile } from "node:fs/promises";
import path from "node:path";
import { promisify } from "node:util";
import { fileURLToPath } from "node:url";

const execFileAsync = promisify(execFile);
const projectRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const fixtureRoot = path.join(projectRoot, "fixtures", "stress", "media");
const sourcePath = path.join(fixtureRoot, "h264-aac-flac-128m.mp4");
const fixturePath = path.join(fixtureRoot, "h264-elementary-128m.h264");
const minimumBytes = 128 * 1024 * 1024;
const generatedContainers = ["3gp", "mp4", "mov", "mkv", "mpegts", "flv"]
  .map((extension) => path.join(fixtureRoot, `h264-aac-flac-128m.${extension}`));

await mkdir(fixtureRoot, { recursive: true });
try {
  await execFileAsync(
    "node",
    ["scripts/generate-container-flac-stress-fixtures.mjs"],
    { cwd: projectRoot, windowsHide: true, maxBuffer: 16 * 1024 * 1024 },
  );
  const sourceManifest = JSON.parse(await readFile(`${sourcePath}.json`, "utf8"));
  await execFileAsync(
    "ffmpeg",
    [
      "-hide_banner", "-loglevel", "error", "-nostdin", "-y",
      "-i", sourcePath, "-map", "0:v:0", "-c:v", "copy",
      "-bsf:v", "h264_mp4toannexb", "-fflags", "+bitexact",
      "-f", "h264", fixturePath,
    ],
    { cwd: projectRoot, windowsHide: true, maxBuffer: 16 * 1024 * 1024 },
  );

  const fixtureStat = await stat(fixturePath);
  if (fixtureStat.size < minimumBytes) {
    throw new Error(`Generated H.264 is ${fixtureStat.size} bytes; expected at least ${minimumBytes}.`);
  }
  const { stdout } = await execFileAsync(
    "ffprobe",
    ["-v", "error", "-count_frames", "-show_format", "-show_streams", "-of", "json", fixturePath],
    { cwd: projectRoot, windowsHide: true, maxBuffer: 16 * 1024 * 1024 },
  );
  const probe = JSON.parse(stdout);
  const video = probe.streams.find((stream) => stream.codec_type === "video");
  const [frameRateNumerator, frameRateDenominator] = String(video?.avg_frame_rate)
    .split("/").map(Number);
  const decodedVideoFrames = Number(video?.nb_read_frames);
  const durationSeconds = decodedVideoFrames * frameRateDenominator / frameRateNumerator;
  if (
    probe.format?.format_name !== "h264" ||
    probe.streams.length !== 1 ||
    video?.codec_name !== "h264" ||
    video?.width !== 1282 ||
    video?.height !== 536 ||
    decodedVideoFrames !== 1560 ||
    !Number.isFinite(durationSeconds)
  ) {
    throw new Error("Generated stress fixture is not the expected 1282x536 H.264 elementary stream.");
  }
  await writeFile(
    `${fixturePath}.json`,
    `${JSON.stringify({
      generatedBy: "scripts/generate-h264-elementary-stress-fixtures.mjs",
      source: "fixtures/stress/media/h264-aac-flac-128m.mp4",
      sourceSha256: sourceManifest.sha256,
      containerDurationSeconds: 65,
      durationSeconds,
      decodedVideoFrames,
      bytes: fixtureStat.size,
      sha256: await hashFile(fixturePath),
      probe,
    }, null, 2)}\n`,
    "utf8",
  );
  process.stdout.write(`${fixturePath}\n`);
} catch (error) {
  await rm(fixturePath, { force: true });
  await rm(`${fixturePath}.json`, { force: true });
  for (const containerPath of generatedContainers) {
    await rm(containerPath, { force: true });
    await rm(`${containerPath}.json`, { force: true });
  }
  throw error;
}

async function hashFile(filePath) {
  const hash = createHash("sha256");
  for await (const chunk of createReadStream(filePath, { highWaterMark: 4 * 1024 * 1024 })) {
    hash.update(chunk);
  }
  return hash.digest("hex");
}
