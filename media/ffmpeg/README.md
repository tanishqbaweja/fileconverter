...
`.awb` input in 3GP/ISOBMFF. It exposes WAV, FLAC, AIFF, MP3, AAC, Opus, Ogg Vorbis, and WMA2 destinations, retains
...
32 kb/s rather than needlessly upsampling either source to 32 kHz. Opus uses
the established complexity-0 64 kb/s VBR profile and preserves 16 kHz internal
processing while Ogg carries the standard 48 kHz Opus clock.
AAC uses the established fastest quality-valid search coder and reduced tool
set, preserves 16 kHz, and writes raw AAC-LC through bounded ADTS packets.
Vorbis uses the established reference quality-4 VBR profile and preserves
16 kHz through the same fixed-memory FIFO and direct Ogg output path.
WMA2 uses the measured fastest quality-valid speech policy: 16 kHz-and-below
mono input is resampled to 32 kHz and encoded at 64 kb/s, while other input
retains the established 48 kHz/320 kb/s profile. The 32 kHz speech policy was
1.43x faster, 88.9% smaller, and about 0.76 dB higher-ASDR than the prior
48 kHz/320 kb/s policy in the controlled one-hour benchmark.
