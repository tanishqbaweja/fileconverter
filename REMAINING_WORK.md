...
| P-04 | Broadest technically practical mainstream format coverage | Partially implemented | 388 public passed profiles across all named categories in `TESTED.md`; exact gaps are listed at its end; HEIF/HEIC and SVG now have explicit evidence-backed boundaries | Complete feasibility audits and implement or defensibly reject the remaining camera-raw and media combinations below. |
...
3. Perform bounded feasibility work for camera raw and the missing media
   combinations. Revisit the evidenced HEIF/HEIC exclusion only when its legal,
   security, and fixed-memory conditions change. Promote only fully evidenced routes.
