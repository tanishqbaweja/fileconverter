...
| P-04 | Broadest technically practical mainstream format coverage | Partially implemented | 388 public passed profiles across all named categories in `TESTED.md`; exact gaps are listed at its end; HEIF/HEIC, camera RAW, and SVG now have explicit evidence-backed boundaries | Implement or defensibly reject the remaining media combinations below. |
...
| P-08 | Optimize conversion speed without weakening correctness, privacy, fidelity, cleanup, or memory | Partially implemented | `TESTED.md` contains measured accepted and rejected optimizations. `evidence/remux-performance-audit-2026-08-28.json` records an identical three-run 2.958 GB A/B: the retained 1 MiB direct specialist's 36.064 s median is 16.4% faster than the 256 KiB candidate, with identical output SHA-256 and both under 250 MiB. `evidence/compatible-webm-copy-2026-09-07.json` accepts standards-compatible AV1/VP8/VP9 packet copy: a 170,427,228-byte VP9/Opus source passed 3/3 in 1.42–1.79 seconds at 206.7 MiB worst complete-Chromium incremental private memory, while exact packet/decode, I/O, cleanup, isolated-build, and no-Docker reproduction gates passed. | Continue benchmark-before/after work per remaining route; record rejected candidates here and in `TESTED.md`. Do not invent a speedup where the prior route was blocked rather than slower. |
...
| M-04 | Mainstream containers and practical codecs named by the specification | Partially implemented | Extensive MKV/MP4/MOV/3GP/MPEG-TS/FLV/AVI/WebM/OGV and H.264/HEVC/VP8/VP9/AV1/MPEG-2/MPEG-4 routes are public. The accepted compatible-WebM path now losslessly packet-copies Matroska AV1, VP8, or VP9 plus Opus/Vorbis, automatically prefers that path, and passed exact small-browser, three-run 170,427,228-byte VP9, 206.7 MiB memory, cleanup, and exact no-Docker reproduction gates in `evidence/compatible-webm-copy-2026-09-07.json`. A guarded, non-public `mkv-to-ogv` candidate now maps Theora/Vorbis to Ogg profile 36; the reusable native feasibility test proves exact compressed-packet retention and full decode while deleting all temporary files. | Build the candidate on the hosted no-Docker path, prove only `within-remux.wasm` changes, then run production-browser packet/decode, cancellation, write-failure, three-run >128 MiB memory, cleanup, and exact-reproduction gates before promotion. Investigate AVI output and the remaining 3GP, AV1, MPEG-2 audio/codec combinations and elementary/raw outputs listed in `TESTED.md`. |
...
### 2026-09-07 — guarded Matroska Theora/Vorbis to OGV candidate
...
- Remux profile 36 is compiled only into `within-remux` behind
  ...
  pictures, chapters, and unrepresentable rotation. Worker and planner source
  are wired, focused planner/registry tests pass 60/60, TypeScript and lint pass,
  and the route remains absent from the public registry until the hosted build
  and complete production-browser acceptance gates pass.
