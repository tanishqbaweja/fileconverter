...
in 0.861-0.924 seconds (175.9-188.9 MiB/s), used fixed 32 MiB Wasm, and peaked
...
The compatible Theora/Vorbis Matroska-to-OGV route is a genuine bounded packet
copy, not an extension rename. Its exact packet, full-decode, cancellation,
write-failure, three-run memory, cleanup, and no-Docker build results are in
`evidence/compatible-ogv-copy-2026-09-07.json`.

AVI-to-OGV is a genuine decode-and-re-encode route, not an extension rename.
The isolated libtheora module uses the measured fastest speed level, bounded
quality/width/frame-rate controls, asynchronous 256 KiB input, single-flight
output, and explicitly excludes AVI audio. A deterministic 143,180,538-byte AVI
passed three OPFS and three direct-save Chrome runs at 220.9 MiB and 231.7 MiB
worst incremental private memory, respectively. Exact output, full decode,
visual quality, cancellation, write failure, and cleanup evidence is recorded in
`evidence/avi-to-ogv-browser-2026-09-13.json`.

Raw MPEG-2-to-OGV also performs genuine decode and Theora re-encode. A
136,166,136-byte, 496-second elementary stream produced an independently
decoded 86,555,131-byte OGV with all 11,904 frames in three same-session
Chrome runs per destination mode. Worst complete-Chrome incremental private
memory was 247.789 MiB in OPFS mode and 246.250 MiB for selected-destination
saves, against the unchanged 250 MiB ceiling. A separate maximum-quality
selected-destination run produced 118,761,511 bytes at 239.625 MiB; it was
not represented as a repeated three-run gate. Selected-destination saves use
quota-preflighted browser-private staging followed by one backpressured
256 KiB copy, then delete the stage. The 2.211 MiB worst observed headroom
is narrow and is not a cross-machine promise. Historical failed direct memory
and OPFS harness results remain in
`evidence/m2v-to-ogv-candidate-2026-09-25.json` and the accepted measurements
in `evidence/m2v-to-ogv-staged-save-2026-09-25.json`.
...
without decoding pixels; SVG uses the bounded UTF-8 path. A ZIP package
inspector reads at most the 65,557-byte tail plus a 1 MiB central-directory
...
payloads or displaying embedded names. TIFF, JPEG XL, and non-ZIP archive
families still show generic type/size inspection and destination limitations. A
...
`evidence/package-source-inspection-2026-09-27.json`; remaining scope is
...
AIFF, AMR, AVI, FLAC, FLV, raw H.264, raw HEVC, raw MPEG-2 video, raw MPEG-4 Part 2 M4V,
Matroska, MOV/MP4/3GP, MPEG-TS, MP3, Ogg, and WAV demuxers; IVF, raw H.264, raw
...
Certified AV1, VP8, or VP9 plus Opus or Vorbis in MP4 now uses the same
lossless packet-copy WebM path. A 170,433,726-byte VP9/Opus MP4 passed three
OPFS runs in 1.53–1.86 seconds at 206.2 MiB worst incremental complete-Chromium
private memory and three direct-save runs in 2.30–2.64 seconds at 219.1 MiB.
The output was byte-repeatable, every compressed stream and all 1,440 decoded
video frames/audio samples matched the source, and the 336-sample MP4 Opus
priming case decoded exactly. Direct saves use the reusable asynchronous BYOB
reader; the synchronous reader and 1 MiB coalescing trials were rejected at
316.6 MiB and 358.3 MiB. Hosted no-Docker run
[34899740113](https://github.com/tanishqbaweja/fileconverter/actions/runs/34899740113)
reproduced the published engine byte-for-byte at commit `0c8bc78`, skipped
mismatch upload, cleaned its build tree, and retained zero artifacts. Generated
sources, converted copies, browser profiles, and the obsolete mismatch artifact
were deleted. See
...
Raw Annex B HEVC can instead take a genuine lossy VP8 WebM route. The browser
decodes all 17,282 stress frames, proportionally downscales 1,920×804 video to
640×268, and encodes 600 kbit/s VP8 while explicitly reconstructing elementary-
stream timing at 25 fps. Three 134,752,786-byte Chrome runs completed in
...
- libtheora 1.2.0 official source archive, SHA-256
...
the indexed 405 route IDs, tested sizes, report hashes, repeatability, and memory
...
is LGPL-3.0-or-later. LAME is LGPL-2.0-or-later, OpenCORE AMR is Apache-2.0 licensed, and libopus, libogg, libvorbis, libtheora, and libvpx are
