...
test("the current-browser MP4/MOV/MPEG-TS-to-AVI, MKV-to-MP4, AVI-to-3GP, and AVI-to-MPEG-TS routes use app-owned bounded staging", () => {
  assert.match(protocol, /mode: "staged-handle"/);
  assert.match(
    app,
    /batch\.profile\.id !== "mp4-to-avi"[\s\S]*batch\.profile\.id !== "mov-to-avi"[\s\S]*batch\.profile\.id !== "mpeg-ts-to-avi"[\s\S]*batch\.profile\.id !== "mkv-to-mp4"[\s\S]*batch\.profile\.id !== "avi-to-3gp"[\s\S]*batch\.profile\.id !== "avi-to-mpeg-ts"/,
  );
  ...
  assert.match(worker, /profileId !== "mp4-to-avi" &&\s*profileId !== "mov-to-avi" &&\s*profileId !== "mpeg-ts-to-avi" &&\s*profileId !== "mkv-to-mp4" &&\s*profileId !== "avi-to-3gp" &&\s*profileId !== "avi-to-mpeg-ts"/);
});
