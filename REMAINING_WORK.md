...
| U-08 | Format-specific pre-conversion source inspection beyond media headers | Partially implemented | The UI shows locally detected format, MIME, size, and destination limitations. Bounded media header reads expose streams/codecs/metadata. A fixed 256 KiB UTF-8 prefix inspector reports non-content structural facts for CSV, TSV, NDJSON, JSON, XML, TXT, Markdown, HTML, SRT, WebVTT, ASS/SSA, TTML, and SVG, with exact counts for complete small sources and explicit lower bounds for larger prefixes. A separate fixed 1 MiB binary-header inspector reports dimensions, depth/color form, animation/page structure, and metadata signals for PNG/APNG, JPEG, GIF, WebP, AVIF, BMP, ICO, and classic TIFF without pixel decode; TIFF additionally follows the bounded image-directory chain and reports compression/orientation signals. ZIP, DOCX, XLSX, PPTX, ODT, ODS, ODP, and EPUB use a directory-only inspector capped at the 65,557-byte tail plus a 1 MiB central-directory prefix; it reports entry/size/compression/encryption/path-safety facts and package-specific document/sheet/slide/publication markers without decompressing payloads or rendering embedded names. Production-browser gates passed structured inspection plus zero-upload 2/2, image inspection 1/1, package inspection 1/1, and the complete privacy/offline suite 18/18. Exact scope is in `evidence/structured-source-inspection-2026-09-26.json`, `evidence/image-source-inspection-2026-09-27.json`, and `evidence/package-source-inspection-2026-09-27.json`; the headed DOCX review remains in `evidence/headed-docx-inspection-2026-09-25.json`. | Add bounded JPEG XL frame metadata inspection plus bounded directory metadata for non-ZIP archive families; continue showing representable and excluded elements without full-file buffering. |
