...
Pushed commit `fa25780` passed [no-Docker verify-only run 36105767209](https://github.com/tanishqbaweja/fileconverter/actions/runs/36105767209): production build, public-evidence audit, lint, TypeScript, unit, privacy/offline, and all three browser conversion partitions with independent validators. The scoped [no-Docker FFmpeg reproduction run 36105796022](https://github.com/tanishqbaweja/fileconverter/actions/runs/36105796022) rebuilt and verified the published FFmpeg artifacts byte-for-byte, then removed its build data; its other jobs were intentionally skipped. Publication commit `d9d76a7` then passed the verify, streaming, and image jobs in [run 36152880728](https://github.com/tanishqbaweja/fileconverter/actions/runs/36152880728), but its media job found three stale MPEG-2 test assertions expecting a warning that the worker intentionally replaces with accurate sequence-header timing. The production conversions completed; the assertions were corrected and passed in a focused local browser rerun. Full hosted verification of the correction is pending.
