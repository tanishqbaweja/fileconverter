...
| P-06 | Every public profile remains at or below 250 MiB complete-Chromium incremental private memory | Partially implemented | The compact historical audit has strict three-run reports for 405/405 published profiles, but fresh Chrome 153 M4A-to-AIFF cold diagnostics contradict an unconditional current-browser pass: 260.172 MiB direct and 251.094 MiB OPFS for the same 140,941,469-byte source. `evidence/aiff-current-chrome-process-breakdown-2026-09-20.json` retains process breakdowns and exact outputs. | Reduce the current conversion-renderer working set and repeat three-run acceptance in both destinations; keep current-Chrome compliance unclaimed until it passes. |
