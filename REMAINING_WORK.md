...
| M-08 | Audio tags, embedded artwork, and metadata preservation | Partially implemented | `evidence/media-disclosure-audit-2026-08-28.json` proves all 259 public FFmpeg profiles state metadata/container and route semantics; `evidence/audio-metadata-retention-browser-2026-08-28.json` independently proves title retention for representative WAV, FLAC, AIFF, ALAC/M4A, WMA/ASF, MP3/ID3, Opus/Ogg, and Vorbis/Ogg destinations, with exact PCM checks for lossless routes. The 2026-09-01 implementation cycle added packet-copy preservation of the first JPEG/PNG attached picture for MP3/FLAC only, bounded to 4 MiB, 4096 pixels per side, and 16 megapixels, plus explicit exclusion messages for unsupported, excess, or unrepresentable artwork. The deterministic 64,973-byte `fixtures/media/audio-source-artwork.m4a` covers seven common tags and exact PNG-byte retention; TypeScript, lint, 86/86 unit tests, patch reverse-application, manifest coverage, fixture probing/hash, and protected `test.mkv` hash all pass. First no-Docker run `33435235488` retained no artifact and exposed a misplaced main-remux declaration at Clang compile time; it was corrected. Run `33436034707` then compiled all modules in 10m24s and proved only `within-remux.wasm` differed (`49249417...505b1`); all 18 peer artifacts matched exactly. Its 19 MB download stayed under `work/`, was verified, and was deleted after the candidate copy. Production Chrome reached the new path but rejected valid PNG art because the minimal no-image-decoder build leaves dimensions unset; the rejected candidate is not being published. The fix parses only bounded PNG/JPEG headers, still packet-copies image bytes without decode, and supplies the verified dimensions to MP3/FLAC muxers. | Rebuild `within-remux.wasm` through the no-Docker reproducibility workflow, then rerun the genuine production-Chrome MP3/FLAC preservation and raw-AAC exclusion test, independent full decode/hash validation, three-run bounded-memory stress/cleanup recovery, registry/UI/README disclosure updates, and the exact reproduction rerun. Representative destination coverage still does not prove every public route. |
