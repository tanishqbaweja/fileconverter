...
- **3GP-contained AMR feasibility and focused gate:** native feasibility proved that the exact AMR-NB fixture losslessly remuxes into genuine 3GP and decodes to signed 16-bit WAV/AIFF or lossless FLAC, or re-encodes to MP3, Opus, and reference Vorbis. All five initial repository-local audit outputs were independently probed, fully decoded, hashed, then removed through the explicit cleanup allowlist. Four pending production routes reuse the established bounded audio profiles without changing the Wasm binary: `3gp-to-aiff`, `3gp-to-mp3`, `3gp-to-opus`, and `3gp-to-ogg`; existing generic `3gp-to-wav` and `3gp-to-flac` routes also accept the AMR codec but retain their AAC-only public claims until codec-specific stress evidence passes. The final focused production-browser gate passed 10/10 in 17.1 seconds: all six genuine outputs passed codec, container, duration, channel, sample-rate, exact decoded-PCM or ASDR validation, and four forced destination-write failures removed every partial OPFS output. Raw `3gp-to-aac` remains a distinct AAC packet-copy route and is not incorrectly broadened to AMR transcoding.
- **3GP/AMR stress fixture validation:** the new category generator completed in 399.5 seconds and produced a real 134,230,542-byte 3GP containing 4,194,669 8 kHz mono AMR-NB frames over 83,893.38 seconds. Full native decode matched the raw AMR source PCM SHA-256 `d28633e8e9cbd0c926b63f3d22c01bdb40d1561c95cfab471f55aab43af69fc9` exactly. Category setup creates this source once for six profiles instead of remuxing it repeatedly; cleanup deleted both large raw/3GP media files and retained only their compact manifests. The first final focused invocation found port 3000 already occupied before any test or conversion began; rerunning on isolated port 3107 avoided interfering with that process and passed.
- **FFmpeg manifest-source synchronization:** adding the pending 3GP/AMR routes exposed that the reproducible `build-remux.sh` profile list had fallen behind the published manifest across recent AMR, MP3, AAC, Opus, and Vorbis output milestones. The build source is now synchronized exactly to the published 199-route list, preventing a future clean export from silently dropping declared profiles. This is a manifest/reproducibility correction only; no engine binary changed.
...
- Audio: AMR-WB remains absent; 3GP-contained AMR has focused browser and exact 128 MiB-class fixture evidence but remains test-only until its six routes complete the required three-pass Chrome process-tree memory gate. Broader AAC/ALAC/WMA variants plus user-selectable bitrate, sample-rate, channel-layout, and artwork/tag handling also remain.
