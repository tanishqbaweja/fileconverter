...
| P-08 | Optimize conversion speed without weakening correctness, privacy, fidelity, cleanup, or memory | Partially implemented | `TESTED.md` contains measured accepted and rejected optimizations. `evidence/remux-performance-audit-2026-08-28.json` records an identical three-run 2.958 GB A/B: the retained 1 MiB direct specialist's 36.064 s median is 16.4% faster than the 256 KiB candidate, with identical output SHA-256 and both under 250 MiB. A source-level candidate now broadens the existing fixed-memory WebM remux profile from AV1-only to standards-compatible AV1/VP8/VP9 packet copy and makes generic WebM prefer it before re-encoding. | Finish the hosted no-Docker Wasm rebuild, production-browser VP8/VP9 packet-identity test, identical-input timing comparison, exact reproduction, and cleanup audit before recording this candidate as accepted. Continue benchmark-before/after work per remaining route; record rejected candidates here and in `TESTED.md`. |
...
| M-04 | Mainstream containers and practical codecs named by the specification | Partially implemented | Extensive MKV/MP4/MOV/3GP/MPEG-TS/FLV/AVI/WebM/OGV and H.264/HEVC/VP8/VP9/AV1/MPEG-2/MPEG-4 routes are public. The in-progress compatible-WebM candidate adds lossless Matroska VP8/VP9 handling to the existing AV1 remux profile without adding a decoder or proportional-memory path. | Promote the compatible-WebM candidate only after its pending hosted build and browser validation pass, then investigate the additional OGV, 3GP, AVI, AV1, MPEG-2 audio/codec combinations and elementary/raw outputs listed in `TESTED.md`. |
