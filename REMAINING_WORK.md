...
| U-08 | Format-specific pre-conversion source inspection beyond media headers | Partially implemented | The UI shows locally detected format, MIME, size, and destination limitations. Bounded media header reads expose streams/codecs/metadata. A fixed 256 KiB UTF-8 prefix inspector now reports non-content structural facts for CSV, TSV, NDJSON, JSON, XML, TXT, Markdown, HTML, SRT, WebVTT, ASS/SSA, and TTML, including exact counts for complete small sources and explicit lower bounds for larger prefixes. The focused production-browser gate passed CSV zero-upload conversion plus JSON, HTML, and SRT inspection 2/2; source text was not rendered. The complete privacy/offline suite passed 16/16. Exact scope and cleanup are in `evidence/structured-source-inspection-2026-09-26.json`; the earlier headed DOCX review remains in `evidence/headed-docx-inspection-2026-09-25.json`. | Add bounded structural inspection for packaged documents, archives, ebooks, and image headers; show representable and excluded elements before conversion without full-file buffering. |
