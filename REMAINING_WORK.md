...
| U-08 | Format-specific pre-conversion source inspection beyond media headers | Partially implemented | The UI shows locally detected format, MIME, size, and destination limitations. Bounded media header reads expose streams/codecs/metadata. A fixed 256 KiB UTF-8 prefix inspector reports non-content structural facts for CSV, TSV, NDJSON, JSON, XML, TXT, Markdown, HTML, SRT, WebVTT, ASS/SSA, TTML, and SVG, with exact counts for complete small sources and explicit lower bounds for larger prefixes. A separate fixed 1 MiB binary-header inspector reports dimensions, depth/color form, animation, and metadata signals for PNG/APNG, JPEG, GIF, WebP, AVIF, BMP, and ICO without pixel decode. Production-browser tests passed structured inspection plus zero-upload 2/2 and image inspection 1/1; the complete current privacy/offline suite passed 17/17. Exact scope is in `evidence/structured-source-inspection-2026-09-26.json` and `evidence/image-source-inspection-2026-09-27.json`; the headed DOCX review remains in `evidence/headed-docx-inspection-2026-09-25.json`. | Add bounded structural inspection for packaged documents, archives, ebooks, TIFF, and JPEG XL; show representable and excluded elements before conversion without full-file buffering. |
