...
| P-06 | Every public profile remains at or below 250 MiB complete-Chromium incremental private memory | Partially implemented | The compact audit has strict historical three-run reports for 405/405 public profiles. Chrome 153 exposed an M4A-to-AIFF regression with the general core (260.172 MiB direct, 251.094 MiB OPFS); a route-scoped 16 MiB AIFF specialist now passed matched three-run direct/OPFS sessions and extra cold/cancellation runs on the same 140,941,469-byte source. Worst direct was 246.648 MiB, including a transient updater descendant, leaving only 3.352 MiB headroom. `evidence/aiff-specialist-browser-acceptance-2026-09-20.json` retains exact hashes, outcomes, and caveats. | Continue current-browser regression and other public-route memory audits; one accepted route and the historical ledger do not guarantee every profile on every Chrome version/machine. |
...
- Chrome 153 re-audit on 2026-09-21 invalidated the narrow direct-save margin:
  ...
  1.618-2.663 seconds, but the route is now failed and hidden until a current
  browser implementation passes with adequate headroom. See
...
- The public-evidence manifest gate rejects pending **public** profiles. After
  ...
  now reports 405 public passed/405 compact entries, zero pending profiles,
  zero public non-passing profiles, and zero PDF routes. This does not certify
  the hidden OGV route or close the remaining product audit.
