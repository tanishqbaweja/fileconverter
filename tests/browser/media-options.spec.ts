...
test("MP3 controls update the bounded native conversion request and plan", async ({
  ...
  await expect(
    page.locator('[data-testid="media-conversion-plan"]'),
  ).toContainText("256 kb/s");
  await expect(
    page.locator('[data-testid="media-conversion-plan"]'),
  ).toContainText("44,100 Hz");
  await expect(
    page.locator('[data-testid="media-conversion-plan"]'),
  ).toContainText("mono");
  await expect
...
test("native MP3 conversion honors the selected bitrate, sample rate, and channels", async ({
  ...
  const base64 = await page.evaluate(async (entryName) => {
    if (!entryName)
      throw new Error("The conversion did not expose an OPFS output.");
    const root = await navigator.storage.getDirectory();
    try {
      ...
      for (let offset = 0; offset < bytes.byteLength; offset += 16 * 1024) {
        binary += String.fromCharCode(
          ...bytes.subarray(
            offset,
            Math.min(offset + 16 * 1024, bytes.byteLength),
          ),
        );
      }
      return btoa(binary);
