...
static int stream_codec_is_copy_compatible(const AVStream *stream,
  ...
  if (profile == 36) {
    if (stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
      return stream->codecpar->codec_id == AV_CODEC_ID_THEORA;
    }
    return stream->codecpar->codec_type == AVMEDIA_TYPE_AUDIO &&
           stream->codecpar->codec_id == AV_CODEC_ID_VORBIS;
  }
...
static int within_audio_transcode(int profile, int requested_bit_rate,
  ...
  const int m4a_aac_output = profile == 34;
...
int within_remux(int profile, int audio_bit_rate, int audio_sample_rate,
  ...
  if (profile != 1 && profile != 2 && profile != 12 && profile != 13 &&
      ...
      profile != 26 && profile != 27 && profile != 35
#ifdef WITHIN_OGV_COPY
      && profile != 36
      && profile != 37
      && profile != 38
  ...
  const int amr_copy_output = profile == 35;
  ...
  const char *muxer_name = h264_output
                               ...
                               : aac_output             ? "adts"
                                ...
                                : amr_copy_output        ? "amr"
                                                        : "mp4";
#ifdef WITHIN_OGV_COPY
  if (video_only_output || av1_webm_output || container_ogv_output ||
      container_avi_output) {
#else
...
#ifdef WITHIN_OGV_COPY
  if (container_ogv_output) {
    within_message(
        1,
        "OGV packet-copies every Theora video and Vorbis audio stream; chapters, subtitles, attachments, attached pictures, and incompatible codecs are explicitly excluded.");
  }
...
#ifdef WITHIN_OGV_COPY
  } else if (source_has_display_rotation && container_ogv_output) {
    within_message(
        1,
        "Source display-rotation metadata cannot be represented by OGV and is explicitly excluded; compressed Theora pixels remain unrotated.");
  ...
  if (input_format->nb_chapters > 0 && !matroska_output) {
    within_message(
        ...
        : amr_copy_output
            ? "Source chapters are explicitly excluded from this raw AMR-NB extraction profile."
  ...
  for (unsigned int index = 0; index < input_format->nb_streams; index++) {
...
#ifdef WITHIN_OGV_COPY
        : container_ogv_output || container_avi_output
    ...
    if (selected_stream && !copy_compatible) {
      if (matroska_output) {
        ...
        within_message(
            ...
            "installed.");
      } else if (amr_copy_output) {
        within_message(
            2,
            "Lossless AMR extraction accepts AMR-NB audio. This source audio codec needs a separately verified conversion route.");
      } else {
        within_message(
            2,
    ...
    if (!selected_stream) {
      if (input_stream->disposition & AV_DISPOSITION_ATTACHED_PIC) {
        ...
        within_message(
            1,
            "Only Vorbis audio can be packet-copied into OGV; incompatible source audio was explicitly excluded.");
      } else if (container_ogv_output &&
                 input_stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
        within_message(
            1,
            "Only Theora video can be packet-copied into OGV; an incompatible additional video stream was explicitly excluded.");
      } else if (container_avi_output &&
                 input_stream->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
        within_message(
            1,
            "Only MP3 audio can be packet-copied into AVI; incompatible source audio was explicitly excluded.");
      } else if (container_avi_output &&
                 input_stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
        within_message(
            1,
        ...
        within_message(
            1,
            "The source video stream is explicitly excluded from the audio-only raw AMR-NB output.");
      } else if (amr_copy_output &&
                 input_stream->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
        within_message(
            1,
            "Only the first compatible AMR-NB audio stream is extracted; an additional or incompatible audio stream was explicitly excluded.");
      } else if (container_flv_output &&
                 input_stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
        within_message(
            1,
        ...
        within_message(
            ...
            elementary_output
                ...
                : amr_copy_output
                  ? "The source subtitle stream is explicitly excluded from this raw AMR-NB extraction profile."
        ...
        within_message(1,
                       ...
                       : amr_copy_output
                           ? "The source attachment is explicitly excluded from this raw AMR-NB extraction profile."
        ...
        within_message(
            ...
            elementary_output
                ...
                : amr_copy_output
                  ? "A source stream type unsupported by this raw AMR-NB extraction profile was explicitly excluded."
                : matroska_output
  ...
  if (av1_webm_output || matroska_live_output) {
    ...
    /* OGV streams incrementally as Ogg pages and needs no MOV options. */
  } else if (container_avi_output) {
    /* An eight MiB OpenDML segment caps retained packet-index storage near
       16 MiB even for minimum-size chunks. The reserved master index covers
       16,384 segments (128 GiB at this limit) without late expansion. */
    av_dict_set(&muxer_options, "riff_size_limit", "8388608", 0);
    av_dict_set(&muxer_options, "reserve_index_space", "262176", 0);
#endif
  } else if (profile == 2) {
    av_dict_set(&muxer_options, "movflags",
                "empty_moov+default_base_moof", 0);
