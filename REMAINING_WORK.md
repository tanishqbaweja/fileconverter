...
| T-06 | Headed manual review of real UI success, progress, destination, cancellation, understandable errors, and responsiveness | Partially implemented | Earlier MKV success/direct/cancel/write-failure audit; `evidence/headed-usability-audit-2026-09-05.json` adds eight success flows across audio/video/image/archive/data/document/ebook/subtitle, a two-file batch, quota failure, and permission failure at 390px. Terminal ETA display corrected after screenshot review. | Complete headed reload recovery, mobile encoding controls, keyboard/focus accessibility and final broad review. September 2 runs are UI evidence only; no new independent output or process-tree memory certification is claimed. |
