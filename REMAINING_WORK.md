...
| P-04 | Broadest technically practical mainstream format coverage                                      | Partially implemented                                                             | 404 public passed profiles across all named categories in `TESTED.md`; exact gaps are listed at its end; HEIF/HEIC, camera RAW, and SVG now have explicit evidence-backed boundaries                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Implement or defensibly reject the remaining media combinations below.                                                                                                                                       |
...
| U-08 | Format-specific pre-conversion source inspection beyond media headers | Partially implemented | The UI shows locally detected format, MIME, size, and selected destination limitations. Bounded media header reads expose streams/codecs/metadata. A headed production DOCX-to-text review in `evidence/headed-docx-inspection-2026-09-25.json` found only generic DOCX type/size before conversion; the old fallback incorrectly promised container-stream/codec checks for a document. The wording now accurately describes local validation during conversion, and 2/2 focused browser tests pass. | Add bounded, format-specific structural inspection for document, archive, image, subtitle, and data sources where practical; show representable and excluded elements before conversion without full-file buffering. |
...
| T-10 | CI runs unit, production build, small browser, network privacy, validators, and reproducibility checks                                                                                      | Verified complete for current published code                                                             | The current-branch [verify-only run 36004924350](https://github.com/tanishqbaweja/fileconverter/actions/runs/36004924350) passed build, lint, TypeScript, 243 unit tests, 15 privacy/offline cases, 153 image, 238 streaming, and 553 media browser conversions with independent validators. The separate [reproduce-only run 36002113153](https://github.com/tanishqbaweja/fileconverter/actions/runs/36002113153) passed exact no-Docker rebuild/diff and cleanup for all 11 published engine directories. Both retained zero artifacts. `git diff --name-only d2cbd6b HEAD` shows no engine source or published-engine byte changes after the reproducibility run; the later source change is in the browser cancellation bridge and passed the verification run. Subsequent changes through `6e7c6cf` are tests and documentation/evidence; local 244/244 unit tests and 404/404 evidence-manifest audit passed. | Keep both CI partitions mandatory after code/engine changes; a successful verify-only run alone does not prove engine reproducibility, and a reproduce-only run alone does not prove browser behavior. |
...
- The public-evidence manifest gate rejects pending **public** profiles. After
  ...
  is 404/404 after MKV-to-AVI was withheld on 2026-09-23.
